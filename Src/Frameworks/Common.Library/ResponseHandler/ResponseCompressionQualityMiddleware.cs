...
public class ResponseCompressionQualityMiddleware
{
    ...
    public async Task Invoke(HttpContext context)
    {
        ...
        if (!StringValues.IsNullOrEmpty(encodings)
            && StringWithQualityHeaderValue.TryParseList(encodings, out var encodingsList) && encodingsList.Count > 0)
        {
            ...
            for (var encodingIndex = 0; encodingIndex < encodingsList.Count; encodingIndex++)
                ...
                else
                {
                    ...
                    encodingsWithQuality[encodingIndex] = new StringWithQualityHeaderValue(encodingValue,
                            _encodingQuality.ContainsKey(encodingValue) ? _encodingQuality[encodingValue] : 0.1)
                        .ToString();
                }
